...
const initApp=()=>{
  ...
  clearItems.addEventListener("cliick", (event)=>{
...
parsedList.forEach(itemObj=>{
  const newToDoItem = createNewItem(itemObj._id, itemPbj._item);
...
const buildListItem=(Item)=>{
  ...
  label.textContent=idem.getItem;
...
const addClickListenerToCheckbox=(checkbox)=>{
  checkbox.addElementListener("click", (event)=>{
    toDoList.removeItemFromList(checkbox.id);
    updatePersistantData(toDoList.getList());
...
return document.getElementById(checkboxId).nextElementSibling.
textContent;
}
...
const updateScreenReaderConfirmation = (newEntryText, actionVerb)=>{
  document.getElementById("confirmation").textContent = `${newEntryText} ${ationVerb}.`;
};